...
# Set the `key` prop with `crypto`

When an API returns list items without stable IDs, map over the response once and add `id: crypto.randomUUID()` to each item before storing and rendering:
